...
//   if (dr::mhp::default_comm().rank() == root) {
//     std::vector<int> ref(n);
//     std::iota(ref.begin(), ref.end(), 1);
...
TEST(MhpTests, DistributedVectorSegmentSizeCompare) {
  ...
  // auto dist1 = dr::mhp::distribution();
  auto dist2 = dr::mhp::distribution();
  // DV dv1(n, dist1, seg_size1);
  DV dv2(n, dist2, seg_size2);

  // iota(dv1.begin(), dv1.end(), 0);
  iota(dv2.begin(), dv2.end(), 0);

  // if (dr::mhp::default_comm().rank() == 0) {
  // std::cerr << "dv1:" << dv1 <<std::endl;
  std::cerr << "dv2:" << dv2 << std::endl;
  // }
